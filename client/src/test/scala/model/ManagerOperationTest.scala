...
object ManagerOperationTest {
  ...
  val giornoInSettimanaWithBadIdRegola:Option[List[GiornoInSettimana]] = Option(List(GiornoInSettimana(1,1,4,10),GiornoInSettimana(1,2,2,10),GiornoInSettimana(1,3,3,10)))
  ...
  def f(message:String):Unit = println(message)
}
