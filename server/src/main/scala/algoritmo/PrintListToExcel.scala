...
object PrintListToExcel {
  ...
  def printInfo(dataI:Date,dateF:Date,infoss:List[Info]): Unit ={
    ...
    val newInfo=createObject(dataI,dateF,infoss,info)
    ...
    def _iteraDate(listDate:List[Date],result:List[String]=List.empty):List[String]=listDate match {
      case ::(head, next) if infoDay.exists(_.data.compareTo(head)==0)=>_iteraDate(next,result:+(infoDay.find(_.data.compareTo(head) == 0) match {
        case Some(value) => value match {
          case InfoDay(_, shift, shift2, None, false, false)=> shift.toString+" "+ shift2.toString
          case InfoDay(_, shift, None, None, false, false)=>shift.toString
