...
object AssignmentOperation extends AssignmentOperation {
  ...
  override def initOperationAssignment(algorithmExecute: AlgorithmExecute, infoForAlgorithm: Future[InfoForAlgorithm]): Future[Option[Int]] = {
        infoForAlgorithm.flatMap(info => {
          emit(getFromKey("start-algorithm"))
          emit(getFromKey("terminal-division"))
          info.theoricalRequest.map(terminal => {
            ...
            case (default,actual) => actual._2.flatMap {
              case Some(StatusCodes.SUCCES_CODE) =>
                actual._1.theoricalRequest.headOption.foreach(terminale => emit(getFromKey("terminal").concat(terminale.terminaleId.toString.concat(getFromKey("success")))))
                default
              case _ =>
                actual._1.theoricalRequest.headOption.foreach(terminale => emit(getFromKey("terminal").concat(terminale.terminaleId.toString.concat(getFromKey("error")))))
                default
            }
          }
        })
  ...
  private def startAlgorithm(infoForAlgorithm: InfoForAlgorithm, algorithmExecute: AlgorithmExecute): Future[Option[Int]] = Future {
    ...
    infoForAlgorithm.absence match {
      case Some(value) => infoForAlgorithm.theoricalRequest.headOption.foreach(x => emit(getFromKey("assign-absence").concat(x.terminaleId.toString)))
  ...
  def assignSundayAndSaturday(infoForAlgorithm: InfoForAlgorithm, algorithmExecute: AlgorithmExecute,listInfo5x2: List[Info] = List.empty, listInfo6x1: List[Info] = List.empty): Future[Option[Int]] ={
    infoForAlgorithm.theoricalRequest.headOption.foreach(x => emit(getFromKey("assign-sundays").concat(x.terminaleId.toString)))
    val (driver5x2, driver6x1) = partition(infoForAlgorithm.persons)
 ...
 //A Posto
  ...
  private def groupForDriver(infoForAlgorithm: InfoForAlgorithm, algorithmExecute: AlgorithmExecute,assigned:List[Info]): Future[Option[Int]] ={
    infoForAlgorithm.theoricalRequest.headOption.foreach(x => emit(getFromKey("groups").concat(x.terminaleId.toString)))
 ...
 private def updateLastFree(infoForAlgorithm: InfoForAlgorithm,result:List[Info],date:Date):List[(Option[Int],Int)]={
  ...
  private def assignShifts(infoForAlgorithm: InfoForAlgorithm, algorithmExecute: AlgorithmExecute, result: List[Info]):Future[Option[Int]] = Future{
    infoForAlgorithm.theoricalRequest.headOption.foreach(x => emit(getFromKey("shifts").concat( x.terminaleId.toString)))
  ...
  def assignFreeDays(drivers: List[(StoricoContratto, Persona)], request: Option[List[InfoReq]], result: List[Info], weeks: List[List[Date]], lastFree: List[(Option[Int],Int)]):(Option[List[InfoReq]], List[Info]) = {
    request.foreach(_.headOption.foreach(x => emit(getFromKey("free-days").concat(x.idTerminal.toString))))
    @scala.annotation.tailrec
    def _assignFreeDays(request: Option[List[InfoReq]], weeks: List[List[Date]], lastFree: List[(Option[Int],Int)], resultNew: List[Info] = List.empty):(Option[List[InfoReq]], List[Info]) = weeks match {
      case ::(week,Nil) if week.length<DAYS_IN_WEEK=>
        emitForFreeWeek(week,request)
        val secureLastFree =  lastFree.partition(x=> week.lastOption.exists(date=> x._2<getDayNumber(date)) && x._2!=SUNDAY)
        val anotherDriver = week match {
          case days if days.length==1 => secureLastFree._2.filter(date => date._2 != SUNDAY && date._2!=SATURDAY)
          case _ =>secureLastFree._2.filter(date => date._2 != SUNDAY)
        }
        val weekFreeDays = _assignFreeWeek(week, request,(secureLastFree._1:::anotherDriver.splitAt((secureLastFree._2.length/SATURDAY)*week.length)._1).sortWith(_._2 > _._2))
        (weekFreeDays._1,upsertListInfo(resultNew,weekFreeDays._3))
      case ::(week, next) if week.length<DAYS_IN_WEEK=>
        emitForFreeWeek(week,request)
        val secureLastFree = lastFree.partition(x=>week.headOption.exists(date=>getDayNumber(date)>x._2 && x._2!=0))
  ...
  // A POSTO
  private def assignShiftAndUpdateRequest(person: Persona, assignedForTurn: Option[List[InfoReq]], dateI: Option[Date], dateF: Option[Date], turnoId: Option[Int], result: List[Info], turnoId1: Option[Int] = None):(Option[List[InfoReq]], Info) = {
    val infoDay = dateI.zip(dateF).map{ case (date, date1) => createListDayBetween(date,date1) }.toList.flatten.filter(date => !result.filter(result => person.matricola.equals(result.idDriver))
    ...
    (infoDay._1,Info(person.matricola,None,isFisso = false,DEFAULT_CONTRACT,infoDay._2))
  }
  ...
  def assignShiftForFissi(fissi: List[(StoricoContratto, Persona)], assignedForTurn: Option[List[InfoReq]],dateI: Date, dateF: Date, result: List[Info]): (Option[List[InfoReq]], List[Info]) = {
    ...
    def _assignShiftForFissi(fissi: List[(StoricoContratto, Persona)], assignedForTurn: Option[List[InfoReq]], resultNew: List[Info] = List.empty):(Option[List[InfoReq]], List[Info]) = fissi match {
      case ::(fisso, next) =>
        val assignmentResult = assignShiftAndUpdateRequest(fisso._2,assignedForTurn,Option(dateI),Option(dateF),fisso._1.turnoId,result,fisso._1.turnoId1)
    ...
    def _assignWeekForRotatory(drivers: List[(StoricoContratto, Persona)], allRequest:Option[List[InfoReq]], week: List[Date], resultNew: List[Info] = List.empty):(Option[List[InfoReq]], List[Info]) = drivers match {
      case ::(driver, next) =>
        ...
        if(isPartTime(contracts,driver._1.contrattoId)){
          val assignedWeek = assignShiftAndUpdateRequest(driver._2, allRequest,week.headOption,week.lastOption,shiftToAssign,result)
          _assignWeekForRotatory(next, assignedWeek._1, week,resultNew :+assignedWeek._2)
        }else {
          shiftToAssign match {
            case Some(shift) if shift==6 =>
              val assignedWeek = assignShiftAndUpdateRequest(driver._2, allRequest,week.headOption,week.lastOption,Some(shift-1),result,shiftToAssign)
              _assignWeekForRotatory(next, assignedWeek._1, week,resultNew :+assignedWeek._2)
            case Some(shift) =>
              val assignedWeek = assignShiftAndUpdateRequest(driver._2, allRequest,week.headOption,week.lastOption,shiftToAssign,result,Some(shift+1))
              _assignWeekForRotatory(next, assignedWeek._1, week,resultNew :+assignedWeek._2)
          }
        }
      case Nil => (allRequest, resultNew)
  ...
  private val filterResult:(List[Info],List[Date])=>List[Info]=(result,week)=>result.map(x=>x.copy(infoDay = x.infoDay.filter(res=>week.exists(_.compareTo(res.data)==0))))


  // A POSTO
  private def assignExtraOrdinary(result:List[Info], allRequest:Option[List[InfoReq]],infoForAlgorithm: InfoForAlgorithm, listWeek:List[List[Date]]):(List[Info],Option[List[InfoReq]])={
    val (_,rotatori)= infoForAlgorithm.persons.map(_._2).partition(x=>infoForAlgorithm.allAvailability.toList.flatten.exists(id =>x.disponibilita.contains(id.idDisponibile)))
    @scala.annotation.tailrec
    def _assignExtraOrdinary(listWeek:List[List[Date]], allRequest:Option[List[InfoReq]], resultNew:List[Info]=List.empty):(List[Info],Option[List[InfoReq]])= listWeek match {
      case ::(week, next) =>
        val called = searchShiftWithMoreRequest(week,allRequest,rotatori,filterResult(result,week))
        _assignExtraOrdinary(next,called._2,upsertListInfo(resultNew,called._1))
      case Nil =>(resultNew,allRequest)
    }
      _assignExtraOrdinary(listWeek,allRequest)
  }

  private val filterInfoReq:(Option[List[InfoReq]],List[Date])=>List[InfoReq]=(allRequest,week)=>week.flatMap(dateInWeek=> allRequest.toList.flatten
    .filter(request=> (request.request - request.assigned) >0 && request.data.compareTo(dateInWeek)==0))

  private val verifyRequest:(List[InfoReq],InfoReq)=>Boolean=(assigned,req)=>assigned.exists(res=>res.data.compareTo(req.data)==0 && res.idShift==req.idShift && res.assigned==res.request)
  private val filterRequest:(List[InfoReq],InfoReq)=>List[InfoReq]=(assigned,req)=>assigned.filter(res=> res.data.compareTo(req.data)==0 && res.idShift==req.idShift)
  private val createResponse:(Option[List[InfoReq]],List[InfoReq])=>Option[List[InfoReq]] =(allRequest,resultAssign)=>
    allRequest.map(_.map{
      case infoReq if resultAssign.exists(res => res.data.compareTo(infoReq.data)==0 && res.idShift==infoReq.idShift)=>
        infoReq.copy(assigned = resultAssign.filter(res => res.data.compareTo(infoReq.data)==0 && res.idShift==infoReq.idShift).foldLeft(0)((default,infoReqResult)=>default+infoReqResult.assigned))
      case infoReq => infoReq
    })
  ...
  private def extraDriver(result:List[Info],mapPerson: mutable.Map[Int, Int],infoReq: InfoReq): List[Info] =
    ...
    })&& infoDay.straordinario.isEmpty  && !infoDay.absence && !infoDay.freeDay
      && ( infoDay.shift.contains(sum(infoReq.idShift)) || (infoDay.shift2 match {
      case Some(value) => Some(value).contains(subtracts(infoReq.idShift))
      case None => infoDay.shift.contains(subtracts(infoReq.idShift))
    }))))).filter(info=> info.infoDay.nonEmpty && info.idDriver.exists(id=>mapPerson.filter(_._2<2).toList.map(_._1).contains(id)))

  private def searchShiftWithMoreRequest(week: List[Date], allRequest:Option[List[InfoReq]],drivers:List[Persona],resultWeek:List[Info]):(List[Info],Option[List[InfoReq]]) = {
   val newInfoReq = filterInfoReq(allRequest,week)
    val mapPerson: mutable.Map[Int, Int] = drivers.flatMap(driver=>driver.matricola.map(id=>id->DEFAULT_STRAORDINARIO)).to(mutable.Map)
    emitForFreeWeek(week,allRequest)
    @scala.annotation.tailrec
    def _searchShiftWithMoreRequest(listInfoReq:List[InfoReq], result:List[Info], resultInfoReq:List[InfoReq]=List.empty):(List[Info],List[InfoReq]) = listInfoReq match {
      case _::_ =>
        ...
        if(driverExtra.isEmpty){
          ...
          val assigned = assignExtra(driver,result,req,listInfoReq)
          driver.idDriver.foreach(id=>mapPerson.get(id).foreach(x=> mapPerson.update(id,x+1)))
          if(verifyRequest(assigned._2,req)){
            _searchShiftWithMoreRequest(newInfoReq,assigned._1,resultInfoReq:::filterRequest(assigned._2,req))
          }else{
            _searchShiftWithMoreRequest(assigned._2,assigned._1,resultInfoReq)
          }
        }
      case Nil =>(result,resultInfoReq:::listInfoReq)
    }

    val resultAssign = _searchShiftWithMoreRequest(newInfoReq,resultWeek)
    (resultAssign._1,createResponse(allRequest,resultAssign._2))
  }
  ...
  private def assignExtra(driver:Info,result:List[Info],infoReq: InfoReq, listInfoReq:List[InfoReq]):(List[Info],List[InfoReq]) ={
    (upsertListInfo(result,List(driver.copy(infoDay = driver.infoDay.map{
      case infoDay if infoDay.data.compareTo(infoReq.data)==0 =>InfoDay(infoDay.data,infoDay.shift,infoDay.shift2,Some(infoReq.idShift))
  ...
  private def ruleThirdSaturday(week: List[Date], allRequest:Option[List[InfoReq]],drivers:List[Persona],result:List[Info]):(Option[List[InfoReq]],List[Info])= {
    allRequest.foreach(_.headOption.foreach(x => emit(getFromKey("third-saturday").concat(x.idTerminal.toString))))
      @scala.annotation.tailrec
  ...
  private def searchInfoDayAndReplace(info:List[Info],day:Date): List[Info] =
    info.map(info=> info.copy(infoDay = info.infoDay.map{
  ...
  private val getFromKey: String => String = key =>
    config.getString(key)

  private val emit :String => Unit = message => emitter.sendMessage(message)

  private val emitForFreeWeek: (List[Date],Option[List[InfoReq]]) => Unit = (week,allRequest)  =>
  week.headOption.foreach(date=> allRequest.foreach(_.headOption.foreach(x => emit(getFromKey("free-week").concat(date.toString.concat(getFromKey("terminal").concat( " ".concat(x.idTerminal.toString))))))))
